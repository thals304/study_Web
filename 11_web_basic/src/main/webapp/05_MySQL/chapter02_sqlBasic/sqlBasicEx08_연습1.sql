...
*/
/*
 * 		24.03.28 TIME 12:15-13:00
 * 		TODAY 소감문
 *      예제가 엄청 많아서 힘들었지만 그래도 내가 배운 걸 제대로 복습한
 *		느낌이라 좋았던 것 같다.
 *		그리고 테이블이 워낙 많아 이미지를 보면서 했는데
 *      이미지를 보니까 데이터 베이스의 연결성이 더 보기 쉬웠고 재미있었던 것 같다.
 * */
# database로 employees 사용
USE EMPLOYEES; 
...
SELECT  EMP_N0 , BIRTH_DATE , GENDER
FROM 	EMPLOYEES ;
...
SELECT  SALARY ,
        SALARY + SALARY * 0.1 AS CHANGE_SALARY
FROM	SALARIES ;
...
SELECT  EMP_NO ,
 		SALARY ,
 		SALARY * 0.033 AS TAX,
 		SALARY - SALARY * 0.033 AS REAL_SALARY
FROM	SALARIES ;
...
# 30) 이름에 'o'가 포함되어 있고 마지막글자가 'o'가 아닌 사원의 사원번호 , 이름을 조회하기.(EMPLOYEES) -> 조건 합치면 o%라고 해도 되나?
SELECT 	EMP_NO , FIRST_NAME , LAST_NAME
FROM	EMPLOYEES 
WHERE 	LAST_NAME LIKE '%o%' 
AND     LAST_NAME NOT LIKE '%o';
        
# 31) 사원 번호와 입사일을 조회하기 (입사일이 빠른 사람 순서 = 년도 숫자가 작은 것) (EMPLOYEES)
SELECT   EMP_NO , HIRE_DATE
FROM	 EMPLOYEES 

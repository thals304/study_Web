...
/*
 * 24.04.08 TIME 19:13-19:33
 * TODAY 소감문
 * 일단 UPDATE 테이블명 SET 변경할 사항 WHERE
 *      DELETE FROM 테이블명 WHERE 을 까먹었다는 심각성을 느꼈다.
 * 앞으로 5월까지 데이터베이스 시험 준비를 해야 하는데 정신 좀 차려야 될 것 같다.
 * 
 * */
USE DML_PRACTICE;       
...
SELECT *
FROM   CAR 
WHERE  SUBSTRING(REG_DT, 1, 4) = "2021";
...
SELECT * FROM CAR WHERE CAR_NM = "아반떼";
...
SELECT * FROM CAR WHERE CAR_NM = "코란도";

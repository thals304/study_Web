/*
 *  24.04.01 TIME 12:30-13:43
 *  TODAY 소감문
 *  일단 테이블의 연결성을 이해하는게 가장 중요했던 것 같다.
 *  그 다음에는 그냥 연결시키고 나면 내가 원래 했었던 대로 코드를 짜면 되었었다.
 *  문제가 많아 시간이 많이 걸렸지만 그래도 부족한 점도 알게 되고 자신감도 조금 더 생겼고
 *  그리고 정답예시를 보고 공부하는 시간도 갖게되어 좋았던 것 같다.
 * 
 * */
...
SELECT    P.PRODUCT_CD , 
          SUM(O.ORDER_GOODS_QTY) AS SUM_GOODS_QTY
FROM      TB_ORDER O
JOIN      TB_MEMBER M
ON        O.MEMBER_ID = M.MEMBER_ID
JOIN      TB_PRODUCT P
...
JOIN     TB_PRODUCT P
ON       O.PRODUCT_CD = P.PRODUCT_CD
WHERE    SUBSTRING(O.ORDER_DT, 1 , 4)
GROUP BY P.PRODUCT_NM
ORDER BY SUM_GOODS_QTY DESC
LIMIT    3;
...
JOIN   TB_PRODUCT P
ON     O.PRODUCT_CD = P.PRODUCT_CD
WHERE  O.DELIVERY_STATUS = '배송중'
GROUP BY M.RESIDENCE
ORDER BY M.RESIDENCE ASC;
...
JOIN   TB_PRODUCT P
ON     O.PRODUCT_CD = P.PRODUCT_CD
WHERE  P.PRODUCT_NM = 'Apple 2020 맥북 에어 13';
...
# 상품의 이름이 ㄱ ~ ㅎ순으로 출력이 안되는거 같은데? 수량이 큰것부터 -> 이름순
SELECT   P.PRODUCT_NM ,
         SUM(O.ORDER_GOODS_QTY) AS SUM_GOODS_QTY 
FROM     TB_ORDER O
JOIN     TB_MEMBER M
...
WHERE    P.PRICE BETWEEN 1000000 AND 2000000
GROUP BY P.PRODUCT_NM
HAVING   SUM_GOODS_QTY >= 5
ORDER BY SUM_GOODS_QTY DESC , P.PRODUCT_NM ASC;


# 10) 한번에 주문 수량이 10개 이상인 상품 정보 모두와 주문수량 조회하기. -> 정답예시 확인해보기
# 상품 정보 모두를 어떻게 조회 하지? P.*으로 가능
SELECT P.* ,
       O.ORDER_GOODS_QTY
FROM   TB_ORDER O
JOIN   TB_MEMBER M
...
JOIN   TB_PRODUCT P
ON     O.PRODUCT_CD = P.PRODUCT_CD
WHERE  O.ORDER_GOODS_QTY >= 10;


# 11) 2020년 동안 판매된 매출총액 조회하기.(가격 * 개수 + 배송비)
SELECT SUBSTRING(O.ORDER_DT, 1, 10) ,
       P.PRICE * O.ORDER_GOODS_QTY + P.DELIVERY_PRICE AS TOTAL_PRICE
FROM   TB_ORDER O
JOIN   TB_MEMBER M
ON     O.MEMBER_ID = M.MEMBER_ID
JOIN   TB_PRODUCT P
ON     O.PRODUCT_CD = P.PRODUCT_CD
WHERE  SUBSTRING(O.ORDER_DT, 1, 4) = '2020'; 
...
SELECT   P.PRODUCT_NM , O.DELIVERY_STATUS 
FROM     TB_ORDER O
JOIN     TB_MEMBER M
ON       O.MEMBER_ID = M.MEMBER_ID
JOIN     TB_PRODUCT P
ON       O.PRODUCT_CD = P.PRODUCT_CD
WHERE    M.MEMBER_NM = '메르켈';


# 18) '메르켈' 회원의 주문금액 총합을 조회하기.
# 주문금액 총합 : 상품 가격 * 개수 + 딜리버리?
